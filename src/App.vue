...
const navSections = computed(() => {
  if (authStore.isManager) {
    return [
      { label: null, items: [{ title: 'Dashboard', to: '/dashboard', icon: 'mdi-view-dashboard-variant' }] },
      {
        label: 'Analytics',
        items: [
...
<template>
  <v-app style="background: var(--pos-bg)">
    <template v-if="showShell">
      ...
      <aside
        class="pos-sidebar"
        :class="{ 'pos-sidebar--collapsed': isCollapsed && !isMobile, 'pos-sidebar--open': mobileOpen }"
      >
        <!-- Brand -->
        <div class="sidebar-brand" :class="{ 'sidebar-brand--collapsed': isCollapsed && !isMobile }">
        ...
        <div v-if="authStore.isCashier && (!isCollapsed || isMobile)" class="cart-widget">
          ...
          <button class="cart-new-btn" @click="store.startNewOrder(); mobileOpen = false">
            New Order
          </button>
        </div>

        <!-- Sandbox badge -->
        <div class="sidebar-footer" :class="{ 'sidebar-footer--collapsed': isCollapsed && !isMobile }">
      ...
      <div class="pos-main" :class="{ 'pos-main--collapsed': isCollapsed && !isMobile }">
        ...
        <header class="pos-appbar">
          ...
          <div class="appbar-right">
            <div class="role-chip" :class="authStore.isManager ? 'role-chip--mgr' : 'role-chip--cshr'">
              <v-icon :icon="authStore.isManager ? 'mdi-shield-account' : 'mdi-account-badge-outline'" size="12" />
              <span>{{ authStore.isManager ? 'Manager' : 'Cashier' }}</span>
            </div>

            <div class="user-wrap">
              <button class="user-avatar" @click="userMenuOpen = !userMenuOpen">{{ userInitials }}</button>
...
@media (min-width: 1024px) {
  .pos-sidebar { transform: translateX(0) !important; }
  .pos-sidebar--collapsed { width: 80px; }
}
...
.sidebar-brand--collapsed { justify-content: center; padding: 20px 12px 12px; }
...
.brand-name { font-weight: 700; font-size: 14px; color: var(--pos-ink); line-height: 1.2; white-space: nowrap; }
.brand-sub { font-size: 10px; color: var(--pos-muted); letter-spacing: 0.1em; text-transform: uppercase; margin-top: 1px; }
...
.nav-divider { height: 1px; background: var(--pos-border); margin: 10px auto; width: 32px; }
...
.nav-item:hover { background: var(--pos-border); color: var(--pos-ink); }
.nav-item--active { background: var(--pos-accent-soft) !important; color: var(--pos-accent-strong) !important; }
...
.cart-eyebrow { font-size: 9px; letter-spacing: 0.22em; text-transform: uppercase; opacity: 0.6; }
.cart-total { font-size: 18px; font-weight: 700; font-family: var(--pos-mono); margin-top: 4px; }
.cart-meta { font-size: 11px; opacity: 0.65; margin-top: 2px; }
.cart-new-btn {
  margin-top: 10px;
  width: 100%;
  padding: 7px 0;
...
.cart-new-btn:hover { background: rgba(255,255,255,0.2); }

/* ── Sidebar footer ── */
.sidebar-footer { padding: 8px 12px 16px; flex-shrink: 0; }
...
.sandbox-text { font-size: 11px; font-weight: 600; color: #059669; white-space: nowrap; }
...
.pos-sidebar:hover .collapse-btn { opacity: 1; }
...
@media (max-width: 1023px) { .pos-main { margin-left: 0 !important; } }
.pos-main--collapsed { margin-left: 80px; }
...
.appbar-title { font-weight: 700; font-size: 17px; letter-spacing: -0.01em; color: var(--pos-ink); }
...
.menu-btn:hover { background: var(--pos-border); }
...
.role-chip--mgr { background: rgba(245,158,11,0.08); color: #b45309; border-color: rgba(245,158,11,0.3); }
.role-chip--cshr { background: #f9fafb; color: var(--pos-muted); border-color: var(--pos-border); }
...
.user-overlay { position: fixed; inset: 0; z-index: 40; }
...
.ud-top { display: flex; align-items: center; gap: 10px; padding: 14px; }
.ud-avatar {
  width: 34px; height: 34px; border-radius: 50%;
  background: var(--pos-navy); color: #fff;
  font-weight: 700; font-size: 11px;
  display: grid; place-items: center; flex-shrink: 0;
}
.ud-name { font-size: 13px; font-weight: 600; color: var(--pos-ink); }
.ud-role { font-size: 11px; color: var(--pos-muted); text-transform: capitalize; margin-top: 1px; }
.ud-divider { height: 1px; background: var(--pos-border); }
.ud-signout {
...
.ud-signout:hover { background: rgba(239,68,68,0.06); }

/* ── Page ── */
.pos-page { flex: 1; overflow: auto; background: var(--pos-bg); }
.pos-page--full { display: flex; flex-direction: column; height: calc(100vh - 60px); overflow: hidden; }
...
@media (max-width: 960px) { .pos-content { padding: 16px; } }
</style>
